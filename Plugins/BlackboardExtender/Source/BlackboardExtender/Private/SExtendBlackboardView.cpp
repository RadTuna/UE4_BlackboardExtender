...
#include "SExtendBlackboardView.h"
...
FName FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId() 
{ 
	static FName Type("FEdGraphSchemaAction_BlackboardEntry"); return Type; 
}

FName FEdGraphSchemaAction_ExtendBlackboardEntry::GetTypeId() const 
{ 
	return StaticGetTypeId(); 
}

FEdGraphSchemaAction_ExtendBlackboardEntry::FEdGraphSchemaAction_ExtendBlackboardEntry( UBlackboardData* InBlackboardData, FBlackboardEntry& InKey, bool bInIsInherited )
...
void FEdGraphSchemaAction_ExtendBlackboardEntry::Update()
{
...
class SBehaviorTreeBlackboardItem : public SGraphPaletteItem
{
	...
	void Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData)
	{
		...
		check(GraphAction->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
		TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> BlackboardEntryAction = StaticCastSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>(GraphAction);
...
private:
	void GetPaletteItemIcon(TSharedPtr<FEdGraphSchemaAction> InGraphAction, FSlateBrush const*& OutIconBrush)
	{
		...
		check(InGraphAction->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
		TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> BlackboardEntryAction = StaticCastSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>(InGraphAction);
	...
	{
		check(ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
		
		const FString AsString = *NewText.ToString();

		if (AsString.Len() > NAME_SIZE)
		{
			UE_LOG(LogTemp, Error, TEXT("%s is not a valid Blackboard key name. Needs to be shorter than 1024 characters."), *NewText.ToString());
			return;
		}

		TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> BlackboardEntryAction = StaticCastSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>(ActionPtr.Pin());
	...
	FText GetDebugTextValue(TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> BlackboardEntry) const
	{
...
void SExtendBlackboardView::AddReferencedObjects( FReferenceCollector& Collector )
{
...
void SExtendBlackboardView::Construct(const FArguments& InArgs, TSharedRef<FUICommandList> InCommandList, UBlackboardData* InBlackboardData)
{
	...
	CommandList->MapAction(
		FExtenderDebuggerCommands::Get().CurrentValues,
		FUIAction(
			FExecuteAction::CreateSP(this, &SExtendBlackboardView::HandleUseCurrentValues),
			FCanExecuteAction::CreateSP(this, &SExtendBlackboardView::IsDebuggerPaused),
			FIsActionChecked::CreateSP(this, &SExtendBlackboardView::IsUsingCurrentValues),
			FIsActionButtonVisible::CreateSP(this, &SExtendBlackboardView::IsDebuggerActive)
			)
		);

	CommandList->MapAction(
		FExtenderDebuggerCommands::Get().SavedValues, 
		FUIAction(
			FExecuteAction::CreateSP(this, &SExtendBlackboardView::HandleUseSavedValues),
			FCanExecuteAction::CreateSP(this, &SExtendBlackboardView::IsDebuggerPaused),
			FIsActionChecked::CreateSP(this, &SExtendBlackboardView::IsUsingSavedValues),
			FIsActionButtonVisible::CreateSP(this, &SExtendBlackboardView::IsDebuggerActive)
			)
		);
	...
	{
		ToolbarBuilder.AddToolBarButton(FExtenderDebuggerCommands::Get().CurrentValues);
		ToolbarBuilder.AddToolBarButton(FExtenderDebuggerCommands::Get().SavedValues);
	}
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(this, &SExtendBlackboardView::GetDebugTimeStampText)
					.Visibility(this, &SExtendBlackboardView::GetDebuggerToolbarVisibility)
				]
			]
			+SVerticalBox::Slot()
			.FillHeight(1.0f)
			[
				...
				.OnCreateWidgetForAction(this, &SExtendBlackboardView::HandleCreateWidgetForAction)
				.OnCollectAllActions(this, &SExtendBlackboardView::HandleCollectAllActions)
				.OnGetSectionTitle(this, &SExtendBlackboardView::HandleGetSectionTitle)
				.OnActionSelected(this, &SExtendBlackboardView::HandleActionSelected)
				.OnContextMenuOpening(this, &SExtendBlackboardView::HandleContextMenuOpening, InCommandList)
				.OnActionMatchesName(this, &SExtendBlackboardView::HandleActionMatchesName)
...
TSharedRef<SWidget> SExtendBlackboardView::HandleCreateWidgetForAction(FCreateWidgetForActionData* const InCreateData)
{
	return SNew(SBehaviorTreeBlackboardItem, InCreateData)
		...
		.OnGetDisplayCurrentState(this, &SExtendBlackboardView::IsUsingCurrentValues)
		.OnBlackboardKeyChanged(OnBlackboardKeyChanged);
}

void SExtendBlackboardView::HandleCollectAllActions( FGraphActionListBuilderBase& GraphActionListBuilder )
{
	if(BlackboardData != nullptr)
	{
		for(auto& ParentKey : BlackboardData->ParentKeys)
		{
			GraphActionListBuilder.AddAction( MakeShareable(new FEdGraphSchemaAction_ExtendBlackboardEntry(BlackboardData, ParentKey, true)) );
		}

		for(auto& Key : BlackboardData->Keys)
		{
			GraphActionListBuilder.AddAction( MakeShareable(new FEdGraphSchemaAction_ExtendBlackboardEntry(BlackboardData, Key, false)) );
		}
	}
}

FText SExtendBlackboardView::HandleGetSectionTitle(int32 SectionID) const
{
	switch(SectionID)
	{
...
void SExtendBlackboardView::HandleActionSelected(const TArray< TSharedPtr<FEdGraphSchemaAction> >& SelectedActions, ESelectInfo::Type InSelectionType) const
{
	if (InSelectionType == ESelectInfo::OnMouseClick  || InSelectionType == ESelectInfo::OnKeyPress || SelectedActions.Num() == 0)
	{
		if(SelectedActions.Num() > 0)
		{
			check(SelectedActions[0]->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
			TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> BlackboardEntry = StaticCastSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>(SelectedActions[0]);
			OnEntrySelected.ExecuteIfBound(&BlackboardEntry->Key, BlackboardEntry->bIsInherited);
		}
	}
}

TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> SExtendBlackboardView::GetSelectedEntryInternal() const
{
	...
	{
		check(SelectedActions[0]->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
		return StaticCastSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>(SelectedActions[0]);
	}

	return TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry>();
}

int32 SExtendBlackboardView::GetSelectedEntryIndex(bool& bOutIsInherited) const
{
	TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> Entry = GetSelectedEntryInternal();
	if(Entry.IsValid())
	{
...
FBlackboardEntry* SExtendBlackboardView::GetSelectedEntry(bool& bOutIsInherited) const
{
	TSharedPtr<FEdGraphSchemaAction_ExtendBlackboardEntry> Entry = GetSelectedEntryInternal();
	if(Entry.IsValid())
	{
...
void SExtendBlackboardView::SetObject(UBlackboardData* InBlackboardData)
{
	BlackboardData = InBlackboardData;
	GraphActionMenu->RefreshAllActions(true);
}

TSharedPtr<SWidget> SExtendBlackboardView::HandleContextMenuOpening(TSharedRef<FUICommandList> ToolkitCommands) const
{
...
void SExtendBlackboardView::FillContextMenu(FMenuBuilder& MenuBuilder) const
{

}

TSharedPtr<FExtender> SExtendBlackboardView::GetToolbarExtender(TSharedRef<FUICommandList> ToolkitCommands) const
{
	return TSharedPtr<FExtender>();
}

void SExtendBlackboardView::HandleUseCurrentValues()
{
	bShowCurrentState = true;
}

void SExtendBlackboardView::HandleUseSavedValues()
{
	bShowCurrentState = false;
}

FText SExtendBlackboardView::GetDebugTimeStampText() const
{
	FText TimeStampText;
...
EVisibility SExtendBlackboardView::GetDebuggerToolbarVisibility() const
{
	if(OnIsDebuggerReady.IsBound())
	{
		return OnIsDebuggerReady.Execute() ? EVisibility::Visible : EVisibility::Collapsed;
...
bool SExtendBlackboardView::IsUsingCurrentValues() const
{
...
bool SExtendBlackboardView::IsUsingSavedValues() const
{
	return !IsUsingCurrentValues();
}

bool SExtendBlackboardView::HasSelectedItems() const
{
	bool bIsInherited = false;
	return GetSelectedEntry(bIsInherited) != nullptr;
}

bool SExtendBlackboardView::IsDebuggerActive() const
{
	if(OnIsDebuggerReady.IsBound())
	{
...
bool SExtendBlackboardView::IsDebuggerPaused() const
{
...
bool SExtendBlackboardView::HandleActionMatchesName(FEdGraphSchemaAction* InAction, const FName& InName) const
{
	check(InAction->GetTypeId() == FEdGraphSchemaAction_ExtendBlackboardEntry::StaticGetTypeId());
	FEdGraphSchemaAction_ExtendBlackboardEntry* BlackboardEntryAction = static_cast<FEdGraphSchemaAction_ExtendBlackboardEntry*>(InAction);
