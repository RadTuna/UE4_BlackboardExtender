...
void FFindInBTResult::SetNodeHighlight(bool bHighlight)
{
	...
	{
		UBEBehaviorTreeGraphNode* BTNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode.Get());
		if (BTNode)
		{
...
TSharedRef<SWidget> FFindInBTResult::CreateIcon() const
{
	...
	if (GraphNode.IsValid())
	{
		if (Cast<UBEBehaviorTreeGraphNode_Service>(GraphNode.Get()))
		{
			Brush = FEditorStyle::GetBrush(TEXT("GraphEditor.PinIcon"));
		}
		else if (Cast<UBEBehaviorTreeGraphNode_Decorator>(GraphNode.Get()))
		{
...
void SFindInBT::MatchTokens(const TArray<FString>& Tokens)
{
	...
	for (auto It(Graph->Nodes.CreateConstIterator()); It; ++It)
	{
		...
		if (BTNode)
		{
			...
			{
				UBEBehaviorTreeGraphNode* Service = *ServiceIt;
...
void SFindInBT::MatchTokensInChild(const TArray<FString>& Tokens, UBEBehaviorTreeGraphNode* Child, FSearchResult ParentNode)
{
	if (Child == NULL)
	{
		return;
