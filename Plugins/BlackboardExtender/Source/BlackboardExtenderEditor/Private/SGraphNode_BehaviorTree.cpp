...
FSlateColor SBehaviorTreePin::GetPinColor() const
{
	return 
		...
		(GraphPinObj->PinType.PinCategory == UBEBehaviorTreeEditorTypes::PinCategory_SingleComposite) ? BehaviorTreeColors::Pin::CompositeOnly :
		(GraphPinObj->PinType.PinCategory == UBEBehaviorTreeEditorTypes::PinCategory_SingleTask) ? BehaviorTreeColors::Pin::TaskOnly :
		(GraphPinObj->PinType.PinCategory == UBEBehaviorTreeEditorTypes::PinCategory_SingleNode) ? BehaviorTreeColors::Pin::SingleNode :
...
void SGraphNode_BehaviorTree::Construct(const FArguments& InArgs, UBEBehaviorTreeGraphNode* InNode)
{
...
{
	UBEBehaviorTreeGraphNode* BTGraphNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode);
	UBEBehaviorTreeGraphNode* BTParentNode = BTGraphNode ? Cast<UBEBehaviorTreeGraphNode>(BTGraphNode->ParentNode) : nullptr;
	...
	const bool bIsConnectedTreeRoot = BTGraphNode && BTGraphNode->IsA<UBEBehaviorTreeGraphNode_Root>() && BTGraphNode->Pins.IsValidIndex(0) && BTGraphNode->Pins[0]->LinkedTo.Num() > 0;
	const bool bIsDisconnected = NodeInstance && NodeInstance->GetExecutionIndex() == MAX_uint16;
	const bool bIsService = BTGraphNode && BTGraphNode->IsA(UBEBehaviorTreeGraphNode_Service::StaticClass());
...
{
	UBEBehaviorTreeGraphNode* BTGraphNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode);
	UBEBehaviorTreeGraphNode_Decorator* BTGraph_Decorator = Cast<UBEBehaviorTreeGraphNode_Decorator>(GraphNode);
	...
	else if (BTGraph_Decorator || Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(GraphNode))
	{
		check(BTGraphNode);
	...
	else if (Cast<UBEBehaviorTreeGraphNode_Task>(GraphNode))
	{
		check(BTGraphNode);
	...
	else if (Cast<UBEBehaviorTreeGraphNode_Composite>(GraphNode))
	{
		check(BTGraphNode);
	...
	else if (Cast<UBEBehaviorTreeGraphNode_Service>(GraphNode))
	{
		NodeColor = bIsActiveForDebugger ? BehaviorTreeColors::Debugger::ActiveService : BehaviorTreeColors::NodeBody::Service;
	}
	else if (Cast<UBEBehaviorTreeGraphNode_Root>(GraphNode) && GraphNode->Pins.IsValidIndex(0) && GraphNode->Pins[0]->LinkedTo.Num() > 0)
	{
...
void SGraphNode_BehaviorTree::UpdateGraphNode()
{
	...
	UBEBehaviorTreeGraphNode* BTNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode);
	...
	const FMargin NodePadding = (Cast<UBEBehaviorTreeGraphNode_Decorator>(GraphNode) || Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(GraphNode) || Cast<UBEBehaviorTreeGraphNode_Service>(GraphNode))
		? FMargin(2.0f)
		: FMargin(8.0f);
...
{
	UBEBehaviorTreeGraphNode_Decorator* MyNode = Cast<UBEBehaviorTreeGraphNode_Decorator>(GraphNode);
...
void SGraphNode_BehaviorTree::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode);
	...
	if (BTGraphNode && FBehaviorTreeDebugger::IsPlaySessionPaused())
	{
		...
		if (bHasResult || bHasTriggers)
		{
			...
			UBEBehaviorTreeGraphNode_Decorator* BTGraph_Decorator = Cast<UBEBehaviorTreeGraphNode_Decorator>(GraphNode);
			UBEBehaviorTreeGraphNode_CompositeDecorator* BTGraph_CompDecorator = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(GraphNode);
		...
		if (bHasTriggers)
		{
			...
			for (int32 i = 0; i < DecoratorWidgets.Num(); i++)
			{
				if (DecoratorWidgets[i].IsValid())
				{
					...
					UBEBehaviorTreeGraphNode* ChildNode = Cast<UBEBehaviorTreeGraphNode>(TestSNode->GraphNode);
...
{
	UBEBehaviorTreeGraphNode* StateNode = CastChecked<UBEBehaviorTreeGraphNode>(GraphNode);
...
void SGraphNode_BehaviorTree::AddPin(const TSharedRef<SGraphPin>& PinToAdd)
{
	...
	{
		const bool bIsSingleTaskPin = PinObj && (PinObj->PinType.PinCategory == UBEBehaviorTreeEditorTypes::PinCategory_SingleTask);
		if (bIsSingleTaskPin)
		{
...
{
	UBEBehaviorTreeGraphNode_CompositeDecorator* DecoratorNode = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(GraphNode);
	...
	UBEBehaviorTreeGraphNode_Task* TaskNode = Cast<UBEBehaviorTreeGraphNode_Task>(GraphNode);
...
static UBEBehaviorTreeGraphNode* GetParentNode(UEdGraphNode* GraphNode)
{
	UBEBehaviorTreeGraphNode* BTGraphNode = Cast<UBEBehaviorTreeGraphNode>(GraphNode);
	if (BTGraphNode->ParentNode != nullptr)
	{
		BTGraphNode = Cast<UBEBehaviorTreeGraphNode>(BTGraphNode->ParentNode);
	}
	...
	if (MyInputPin != nullptr && MyInputPin->LinkedTo.Num() > 0)
	{
		...
		if(MyParentOutputPin != nullptr)
		{
			...
			{
				return CastChecked<UBEBehaviorTreeGraphNode>(MyParentOutputPin->GetOwningNode());
			}
		}
	}
...
{
	UBEBehaviorTreeGraphNode* ParentNode = GetParentNode(GraphNode);
...
EVisibility SGraphNode_BehaviorTree::GetIndexVisibility() const
{
	...
	if(GraphNode->IsA(UBEBehaviorTreeGraphNode_Root::StaticClass()))
	{
...
FText SGraphNode_BehaviorTree::GetIndexText() const
{
	...
	if (bShowExecutionIndexInEditorMode || GEditor->bIsSimulatingInEditor || GEditor->PlayWorld != NULL)
	{
		...
		UBEBehaviorTreeGraphNode_CompositeDecorator* CompDecorator = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(GraphNode);
...
void SGraphNode_BehaviorTree::GetOverlayBrushes(bool bSelected, const FVector2D WidgetSize, TArray<FOverlayBrushInfo>& Brushes) const
{
	...
	{
		if (BTNode->bDebuggerMarkBreakpointTrigger || (BTNode->bDebuggerMarkCurrentlyActive && BTNode->IsA(UBEBehaviorTreeGraphNode_Task::StaticClass())))
		{
...
void SGraphNode_BehaviorTree::MoveTo(const FVector2D& NewPosition, FNodeSet& NodeFilter)
{
	...
	{
		UBEBehaviorTreeGraph* BTGraph = BTGraphNode->GetBehaviorTreeGraph();
		if (BTGraph)
		{
