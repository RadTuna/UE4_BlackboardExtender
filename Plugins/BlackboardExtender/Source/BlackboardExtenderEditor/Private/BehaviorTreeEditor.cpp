...
void FBehaviorTreeEditor::InitBehaviorTreeEditor( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UObject* InObject )
{
	...
	if(EditedObjects == nullptr || EditedObjects->Num() == 0)
	{
		...
		InitAssetEditor( Mode, InitToolkitHost, FBlackboardExtenderEditorModule::BlackboardExtenderEditorAppIdentifier, DummyLayout, bCreateDefaultStandaloneMenu, bCreateDefaultToolbar, ObjectsToEdit );
...
void FBehaviorTreeEditor::RestoreBehaviorTree()
{
	...
	UBEBehaviorTreeGraph* MyGraph = Cast<UBEBehaviorTreeGraph>(BehaviorTree->BTGraph);
	const bool bNewGraph = MyGraph == NULL;
	if (MyGraph == NULL)
	{
		BehaviorTree->BTGraph = FBlueprintEditorUtils::CreateNewGraph(BehaviorTree, TEXT("Behavior Tree"), UBEBehaviorTreeGraph::StaticClass(), UEdBEGraphSchema_BehaviorTree::StaticClass());
		MyGraph = Cast<UBEBehaviorTreeGraph>(BehaviorTree->BTGraph);
	...
	if(bNewGraph)
	{
		MyGraph->UpdateAsset(UBEBehaviorTreeGraph::ClearDebuggerFlags | UBEBehaviorTreeGraph::KeepRebuildCounter);
		RefreshBlackboardViewsAssociatedObject();
	}
	else
	{
		MyGraph->UpdateAsset(UBEBehaviorTreeGraph::KeepRebuildCounter);
		RefreshDebugger();
	}
...
TSharedRef<SWidget> FBehaviorTreeEditor::SpawnBlackboardDetails()
{
	...
	const TWeakPtr<SBehaviorTreeBlackboardView> BlackboardViewWeakPtr = BlackboardView;
	FOnGetDetailCustomizationInstance LayoutVariableDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FBlackboardDataDetails::MakeInstance, BlackboardViewWeakPtr, OnGetSelectedBlackboardItemIndex);
...
void FBehaviorTreeEditor::OnSelectedNodesChanged(const TSet<class UObject*>& NewSelection)
{
	...
	else if (DetailsView.IsValid())
	{
		if (Selection.Num() == 0)
		{
			...
			{
				RootNode = Cast<UBEBehaviorTreeGraphNode_Root>(Node);
				if (RootNode != nullptr)
				{
					break;
				}
...
void FBehaviorTreeEditor::GetAbortModePreview(const class UBEBehaviorTreeGraphNode_CompositeDecorator* Node, struct FAbortDrawHelper& Mode0, struct FAbortDrawHelper& Mode1)
{
...
void FBehaviorTreeEditor::OnFinishedChangingProperties(const FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		if(PropertyChangedEvent.Property->GetFName() == TEXT("FlowAbortMode"))
		{
			...
			if (CurrentSelection.Num() == 1)
			{
				...
				{
					UBEBehaviorTreeGraphNode_Decorator* DecoratorNode = Cast<UBEBehaviorTreeGraphNode_Decorator>(*It);
					if (DecoratorNode)
					{
						FAbortDrawHelper Mode0, Mode1;
	...
	if (PropertyChangedEvent.Property && PropertyChangedEvent.Property->GetFName() == TEXT("BehaviorAsset"))
	{
		...
		MyGraph->UpdateAsset(UBEBehaviorTreeGraph::ClearDebuggerFlags);
	}
...
{
	UBEBehaviorTreeGraph* MyGraph = BehaviorTree ? Cast<UBEBehaviorTreeGraph>(BehaviorTree->BTGraph) : NULL;
	if (MyGraph)
	{
...
{
	UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(Node);
	if (MyNode && MyNode->bInjectedNode)
	{
		...
		if (SubtreeTask && SubtreeTask->GetSubtreeAsset())
		{
			...
			IBEBehaviorTreeEditor* ChildNodeEditor = static_cast<IBEBehaviorTreeEditor*>(GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(SubtreeTask->GetSubtreeAsset(), true));
			if (ChildNodeEditor)
			{
				ChildNodeEditor->InitializeDebuggerState(Debugger.Get());
				
				int32 FirstInjectedIdx = INDEX_NONE;
				UBEBehaviorTreeGraphNode* MyParentNode = Cast<UBEBehaviorTreeGraphNode>(MyNode->ParentNode);
	...
	else if (UBEBehaviorTreeGraphNode_CompositeDecorator* Decorator = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(Node))
	{
	...
	else if (UBEBehaviorTreeGraphNode_SubtreeTask* Task = Cast<UBEBehaviorTreeGraphNode_SubtreeTask>(Node))
	{
		if (UBTTask_RunBehavior* RunTask = Cast<UBTTask_RunBehavior>(Task->NodeInstance))
		{
			if (RunTask->GetSubtreeAsset())
			{
				...
				IBEBehaviorTreeEditor* ChildNodeEditor = static_cast<IBEBehaviorTreeEditor*>(GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(RunTask->GetSubtreeAsset(), true));
				if (ChildNodeEditor)
				{
					ChildNodeEditor->InitializeDebuggerState(Debugger.Get());
				}
...
void FBehaviorTreeEditor::OnAddInputPin()
{
	...
	for (FGraphPanelSelectionSet::TConstIterator It(CurrentSelection); It; ++It)
	{
		UBEBehaviorTreeDecoratorGraphNode_Logic* LogicNode = Cast<UBEBehaviorTreeDecoratorGraphNode_Logic>(*It);
		if (LogicNode)
		{
...
void FBehaviorTreeEditor::OnRemoveInputPin()
{
	...
	if (FocusedGraphEd.IsValid())
	{
		...
		if (UBEBehaviorTreeDecoratorGraphNode_Logic* LogicNode = Cast<UBEBehaviorTreeDecoratorGraphNode_Logic>(OwningNode))
		{
...
void FBehaviorTreeEditor::OnGraphEditorFocused(const TSharedRef<SGraphEditor>& InGraphEditor)
{
	...
	FocusedGraphOwner = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(InGraphEditor->GetCurrentGraph()->GetOuter());
...
void FBehaviorTreeEditor::SaveAsset_Execute()
{
	if (BehaviorTree)
	{
		UBEBehaviorTreeGraph* BTGraph = Cast<UBEBehaviorTreeGraph>(BehaviorTree->BTGraph);
		if (BTGraph)
		{
			BTGraph->OnSave();
		}
	...
	// save it
	IBEBehaviorTreeEditor::SaveAsset_Execute();
}
...
void FBehaviorTreeEditor::OnEnableBreakpoint()
{
	...
	{
		UBEBehaviorTreeGraphNode* SelectedNode = Cast<UBEBehaviorTreeGraphNode>(*NodeIt);
...
void FBehaviorTreeEditor::DebuggerSwitchAsset(UBehaviorTree* NewAsset)
{
	if (NewAsset)
	{
		...
		IBEBehaviorTreeEditor* ChildNodeEditor = static_cast<IBEBehaviorTreeEditor*>(GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(NewAsset, true));
		if (ChildNodeEditor)
		{
			ChildNodeEditor->InitializeDebuggerState(Debugger.Get());
		}
...
{
	UBEBehaviorTreeGraph* BTGraph = BehaviorTree ? Cast<UBEBehaviorTreeGraph>(BehaviorTree->BTGraph) : NULL;
	if (BTGraph)
	{
...
{
	UE_CLOG(InClass == nullptr, LogBlackboardExtenderEditor, Error, TEXT("Trying to handle new node of NULL class for Behavior Treee %s ")
