	...
	else if(Result.Node1)
	{
		...
		auto FocusNode = [this](UEdGraphNode* InNode)
		{
			if (InNode)
			{
				...
				UBEBehaviorTreeGraphNode* BTNode = Cast<UBEBehaviorTreeGraphNode>(InNode);
				if (BTNode && BTNode->bIsSubNode)
				{
					...
					{
						UBEBehaviorTreeGraphNode* BTPotentialParentNode = Cast<UBEBehaviorTreeGraphNode>(PotentialParentNode);
