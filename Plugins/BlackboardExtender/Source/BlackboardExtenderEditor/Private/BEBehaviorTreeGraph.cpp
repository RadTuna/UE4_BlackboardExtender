...
UBEBehaviorTreeGraph::UBEBehaviorTreeGraph(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	Schema = UEdBEGraphSchema_BehaviorTree::StaticClass();
}

void UBEBehaviorTreeGraph::UpdateBlackboardChange()
{
	...
	{
		UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(Nodes[Index]);
		...
		if (MyNode)
		{
			...
			for (int32 iDecorator = 0; iDecorator < MyNode->Decorators.Num(); iDecorator++)
			{
				...
				UBEBehaviorTreeGraphNode_CompositeDecorator* CompDecoratorNode = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(MyNode->Decorators[iDecorator]);
...
void UBEBehaviorTreeGraph::UpdateAsset(int32 UpdateFlags)
{
	if (bLockUpdates)
	{
		return;
	...
	UBEBehaviorTreeGraphNode_Root* RootNode = NULL;
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode* Node = Cast<UBEBehaviorTreeGraphNode>(Nodes[Index]);
		...
		if (RootNode == NULL)
		{
			RootNode = Cast<UBEBehaviorTreeGraphNode_Root>(Nodes[Index]);
		}

		UBEBehaviorTreeGraphNode_CompositeDecorator* CompositeDecorator = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(Nodes[Index]);
	...
	{
		UBEBehaviorTreeGraphNode* Node = Cast<UBEBehaviorTreeGraphNode>(RootNode->Pins[0]->LinkedTo[0]->GetOwningNode());
		if (Node)
		{
...
void UBEBehaviorTreeGraph::OnCreated()
{
	Super::OnCreated();
	SpawnMissingNodes();
}

void UBEBehaviorTreeGraph::OnLoaded()
{
	Super::OnLoaded();
...
void UBEBehaviorTreeGraph::Initialize()
{
	Super::Initialize();
	UpdateInjectedNodes();
}

void UBEBehaviorTreeGraph::OnSave()
{
	SpawnMissingNodesForParallel();
	UpdateAsset();
}

void UBEBehaviorTreeGraph::UpdatePinConnectionTypes()
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		...
		const bool bIsCompositeNode = Node->IsA(UBEBehaviorTreeGraphNode_Composite::StaticClass());
		...
		for (int32 iPin = 0; iPin < Node->Pins.Num(); iPin++)
		{
			...
			if (PinCategory == TEXT("Transition"))
			{
				PinCategory = bIsCompositeNode ? 
					...
					UBEBehaviorTreeEditorTypes::PinCategory_SingleComposite;
			}
		}
	}
}

void UBEBehaviorTreeGraph::ReplaceNodeConnections(UEdGraphNode* OldNode, UEdGraphNode* NewNode)
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UEdGraphNode* Node = Nodes[Index];
...
void UBEBehaviorTreeGraph::UpdateVersion_RunBehaviorInSeparateGraph()
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode* Node = Cast<UBEBehaviorTreeGraphNode>(Nodes[Index]);
		if (Node)
		{
			...
			{
				UBEBehaviorTreeGraphNode* NewNode = Cast<UBEBehaviorTreeGraphNode>(StaticDuplicateObject(Node, this, NAME_None, RF_AllFlags, UBEBehaviorTreeGraphNode_SubtreeTask::StaticClass()));
				check(NewNode);
...
void UBEBehaviorTreeGraph::RemoveUnknownSubNodes()
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode* Node = Cast<UBEBehaviorTreeGraphNode>(Nodes[Index]);
...
void UBEBehaviorTreeGraph::UpdateBrokenComposites()
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode_CompositeDecorator* Node = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(Nodes[Index]);
		if (Node)
		{
...
namespace BTGraphHelpers
{
	...
	void CollectDecorators(UBehaviorTree* BTAsset,
		UBEBehaviorTreeGraphNode* GraphNode, TArray<UBTDecorator*>& DecoratorInstances, TArray<FBTDecoratorLogic>& DecoratorOperations,
		bool bInitializeNodes, UBTCompositeNode* RootNode, uint16* ExecutionIndex, uint8 TreeDepth, int32 ChildIdx)
	{
		TMap<UBEBehaviorTreeGraphNode_CompositeDecorator*, FIntIntPair> CompositeMap;
		int32 NumNodes = 0;

		for (int32 i = 0; i < GraphNode->Decorators.Num(); i++)
		{
			UBEBehaviorTreeGraphNode* MyNode = GraphNode->Decorators[i];
			if (MyNode == NULL || MyNode->bInjectedNode)
			{
				continue;
			}

			UBEBehaviorTreeGraphNode_Decorator* MyDecoratorNode = Cast<UBEBehaviorTreeGraphNode_Decorator>(MyNode);
			UBEBehaviorTreeGraphNode_CompositeDecorator* MyCompositeNode = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(MyNode);
		...
		if (bInitializeNodes)
		{
			...
			for (TMap<UBEBehaviorTreeGraphNode_CompositeDecorator*, FIntIntPair>::TIterator It(CompositeMap); It; ++It)
			{
				UBEBehaviorTreeGraphNode_CompositeDecorator* Node = It.Key();
	...
	void InitializeInjectedNodes(UBEBehaviorTreeGraphNode* GraphNode, UBTCompositeNode* RootNode, uint16 ExecutionIndex, uint8 TreeDepth, int32 ChildIdx)
	{
		TMap<UBEBehaviorTreeGraphNode_CompositeDecorator*, FIntIntPair> CompositeMap;
	...
	void VerifyDecorators(UBEBehaviorTreeGraphNode* GraphNode)
	{
		TArray<UBTDecorator*> DecoratorInstances;
		TArray<FBTDecoratorLogic> DecoratorOperations;
	...
	void CreateChildren(UBehaviorTree* BTAsset, UBTCompositeNode* RootNode, const UBEBehaviorTreeGraphNode* RootEdNode, uint16* ExecutionIndex, uint8 TreeDepth)
	{
		if (RootEdNode == NULL)
		{
			return;
		...
		for (int32 PinIdx = 0; PinIdx < RootEdNode->Pins.Num(); PinIdx++)
		{
			...
			{
				UBEBehaviorTreeGraphNode* GraphNode = Cast<UBEBehaviorTreeGraphNode>(Pin->LinkedTo[Index]->GetOwningNode());
				if (GraphNode == NULL)
				{
					continue;
				}
	...
	void ClearRootLevelFlags(UBEBehaviorTreeGraph* Graph)
	{
		for (int32 Index = 0; Index < Graph->Nodes.Num(); Index++)
		{
			UBEBehaviorTreeGraphNode* BTNode = Cast<UBEBehaviorTreeGraphNode>(Graph->Nodes[Index]);
			if (BTNode)
			{
				...
				{
					UBEBehaviorTreeGraphNode* SubNode = BTNode->Decorators[SubIndex];
					if (SubNode)
					{
	...
	void RebuildExecutionOrder(UBEBehaviorTreeGraphNode* RootEdNode, UBTCompositeNode* RootNode, uint16* ExecutionIndex, uint8 TreeDepth)
	{
		if (RootEdNode == NULL)
		{
			return;
		...
		for (int32 PinIdx = 0; PinIdx < RootEdNode->Pins.Num(); PinIdx++)
		{
			...
			{
				UBEBehaviorTreeGraphNode* GraphNode = Cast<UBEBehaviorTreeGraphNode>(SortedPins[Index]->GetOwningNode());
				if (GraphNode == NULL)
				{
					continue;
				}
	...
	void SpawnMissingDecoratorNodes(UBEBehaviorTreeGraphNode* GraphNode, const TArray<UBTDecorator*>& Decorators, const TArray<FBTDecoratorLogic>& DecoratorOps, UBEBehaviorTreeGraph* Graph)
	{
		if (DecoratorOps.Num() == 0)
		{
			for (int32 SubIdx = 0; SubIdx < Decorators.Num(); SubIdx++)
			{
				UBEBehaviorTreeGraphNode* DecoratorNode = NewObject<UBEBehaviorTreeGraphNode_Decorator>(Graph);
		...
		else
		{
			...
			while (Idx < DecoratorOps.Num())
			{
				...
				{
					UBEBehaviorTreeGraphNode_CompositeDecorator* CompositeNode = NewObject<UBEBehaviorTreeGraphNode_CompositeDecorator>(Graph);
	...
	UBEBehaviorTreeGraphNode* SpawnMissingGraphNodesWorker(UBTNode* Node, UBEBehaviorTreeGraphNode* ParentGraphNode, int32 ChildIdx, int32 ParentDecoratorCount, UBEBehaviorTreeGraph* Graph)
	{
		if (Node == nullptr)
		{
			return nullptr;
		...
		UBEBehaviorTreeGraphNode* GraphNode = nullptr;

		UBTCompositeNode* CompositeNode = Cast<UBTCompositeNode>(Node);
		if (Node->IsA(UBTComposite_SimpleParallel::StaticClass()))
		{
			FGraphNodeCreator<UBEBehaviorTreeGraphNode_SimpleParallel> NodeBuilder(*Graph);
			GraphNode = NodeBuilder.CreateNode();
			NodeBuilder.Finalize();
		}
		else if (CompositeNode)
		{
			FGraphNodeCreator<UBEBehaviorTreeGraphNode_Composite> NodeBuilder(*Graph);
			GraphNode = NodeBuilder.CreateNode();
			NodeBuilder.Finalize();
		}
		else if (Node->IsA(UBTTask_RunBehavior::StaticClass()))
		{
			FGraphNodeCreator<UBEBehaviorTreeGraphNode_SubtreeTask> NodeBuilder(*Graph);
			GraphNode = NodeBuilder.CreateNode();
			NodeBuilder.Finalize();
		}
		else
		{
			FGraphNodeCreator<UBEBehaviorTreeGraphNode_Task> NodeBuilder(*Graph);
			GraphNode = NodeBuilder.CreateNode();
			NodeBuilder.Finalize();
		}
		...
		if (CompositeNode)
		{
			...
			{
				UBEBehaviorTreeGraphNode* ServiceNode = NewObject<UBEBehaviorTreeGraphNode_Service>(Graph);
			...
			for (int32 Idx = 0; Idx < CompositeNode->Children.Num(); Idx++)
			{
				...
				UBEBehaviorTreeGraphNode* ChildGraphNode = SpawnMissingGraphNodesWorker(ChildNode, GraphNode,
	...
	UBEBehaviorTreeGraphNode* SpawnMissingGraphNodes(UBehaviorTree* Asset, UBEBehaviorTreeGraphNode* ParentGraphNode, UBEBehaviorTreeGraph* Graph)
	{
		...
		UBEBehaviorTreeGraphNode* GraphNode = SpawnMissingGraphNodesWorker(Asset->RootNode, ParentGraphNode, 0, Asset->RootDecorators.Num(), Graph);
...
void UBEBehaviorTreeGraph::CreateBTFromGraph(UBEBehaviorTreeGraphNode* RootEdNode)
{
	UBehaviorTree* BTAsset = Cast<UBehaviorTree>(GetOuter());
	...
	{
		UBEBehaviorTreeGraphNode* Node = RootEdNode->Decorators[Index];
		if (Node)
		{
...
void UBEBehaviorTreeGraph::CollectAllNodeInstances(TSet<UObject*>& NodeInstances)
{
	Super::CollectAllNodeInstances(NodeInstances);

	for (UEdGraphNode* EdGraphNode : Nodes)
	{
		UBEBehaviorTreeGraphNode* BTGraphNode = Cast<UBEBehaviorTreeGraphNode>(EdGraphNode);
		if (BTGraphNode != nullptr)
		{
			for (UBEBehaviorTreeGraphNode* BTDecoratorGraphNode : BTGraphNode->Decorators)
			{
				...
				if (EdSubGraph != nullptr)
				{
					...
					{
						const UBEBehaviorTreeDecoratorGraphNode_Decorator* DecoratorNode = Cast<const UBEBehaviorTreeDecoratorGraphNode_Decorator>(SubGraphNode);
...
void UBEBehaviorTreeGraph::SpawnMissingNodes()
{
	UBehaviorTree* BTAsset = Cast<UBehaviorTree>(GetOuter());
	if (BTAsset)
	{
		UBEBehaviorTreeGraphNode* RootNode = nullptr;
		for (int32 Idx = 0; Idx < Nodes.Num(); Idx++)
		{
			RootNode = Cast<UBEBehaviorTreeGraphNode_Root>(Nodes[Idx]);
			if (RootNode)
			{
				break;
			}
		...
		UBEBehaviorTreeGraphNode* SpawnedRootNode = BTGraphHelpers::SpawnMissingGraphNodes(BTAsset, RootNode, this);
...
void UBEBehaviorTreeGraph::SpawnMissingNodesForParallel()
{
	UBehaviorTree* BTAsset = Cast<UBehaviorTree>(GetOuter());
	if (BTAsset)
	{
		TArray<UBEBehaviorTreeGraphNode_SimpleParallel*> FixNodes;
		for (int32 Idx = 0; Idx < Nodes.Num(); Idx++)
		{
			UBEBehaviorTreeGraphNode_SimpleParallel* ParallelNode = Cast<UBEBehaviorTreeGraphNode_SimpleParallel>(Nodes[Idx]);
			if (ParallelNode)
			{
		...
		{
			UBEBehaviorTreeGraphNode_SimpleParallel* ParallelNode = FixNodes[Idx];
			...
			if (ParallelInstance)
			{
				...
				{
					UBEBehaviorTreeGraphNode* MainTaskNode = Cast<UBEBehaviorTreeGraphNode>(MainTaskPin->LinkedTo[0]->GetOwningNode());
					if (MainTaskNode)
					{
				...
				FGraphNodeCreator<UBEBehaviorTreeGraphNode_Task> NodeBuilder(*this);
				UBEBehaviorTreeGraphNode_Task* WaitTaskNode = NodeBuilder.CreateNode();
...
void UBEBehaviorTreeGraph::UpdateAbortHighlight(struct FAbortDrawHelper& Mode0, struct FAbortDrawHelper& Mode1)
{
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode* Node = Cast<UBEBehaviorTreeGraphNode>(Nodes[Index]);
...
bool UBEBehaviorTreeGraph::UpdateInjectedNodes()
{
	bool bHasUpdated = false;
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
	{
		UBEBehaviorTreeGraphNode_SubtreeTask* Node = Cast<UBEBehaviorTreeGraphNode_SubtreeTask>(Nodes[Index]);
		if (Node)
		{
...
UEdGraphNode* UBEBehaviorTreeGraph::FindInjectedNode(int32 Index)
{
	for (int32 NodeIdx = 0; NodeIdx < Nodes.Num(); NodeIdx++)
	{
		UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(Nodes[NodeIdx]);
...
void UBEBehaviorTreeGraph::RebuildExecutionOrder()
{
	// initial cleanup & root node search
	UBEBehaviorTreeGraphNode_Root* RootNode = NULL;
	for (int32 Index = 0; Index < Nodes.Num(); ++Index)
...
void UBEBehaviorTreeGraph::RebuildChildOrder(UEdGraphNode* ParentNode)
{
...
#if WITH_EDITOR
void UBEBehaviorTreeGraph::PostEditUndo()
{
	Super::PostEditUndo();
...
namespace BTAutoArrangeHelpers
{
	...
	void AutoArrangeNodes(UBEBehaviorTreeGraphNode* ParentNode, FNodeBoundsInfo& BBoxTree, float PosX, float PosY)
	{
		int32 BBoxIndex = 0;
		...
		if (Pin)
		{
			...
			{
				UBEBehaviorTreeGraphNode* GraphNode = Cast<UBEBehaviorTreeGraphNode>(TempLinkedTo[Idx]->GetOwningNode());
	...
	void GetNodeSizeInfo(UBEBehaviorTreeGraphNode* ParentNode, FNodeBoundsInfo& BBoxTree)
	{
		...
		if (Pin)
		{
			...
			{
				UBEBehaviorTreeGraphNode* GraphNode = Cast<UBEBehaviorTreeGraphNode>(Pin->LinkedTo[Idx]->GetOwningNode());
				if (GraphNode)
				{
...
void UBEBehaviorTreeGraph::AutoArrange()
{
	UBEBehaviorTreeGraphNode* RootNode = nullptr;
	for (int32 Idx = 0; Idx < Nodes.Num(); Idx++)
	{
...
void UBEBehaviorTreeGraph::OnSubNodeDropped()
{
	Super::OnSubNodeDropped();

	FAbortDrawHelper EmptyMode;
	UpdateAsset(UBEBehaviorTreeGraph::ClearDebuggerFlags);
	UpdateAbortHighlight(EmptyMode, EmptyMode);
}

void UBEBehaviorTreeGraph::UpdateVersion()
{
...
void UBEBehaviorTreeGraph::MarkVersion()
{
	GraphVersion = BTGraphVersion::Latest;
	bIsUsingModCounter = true;
}

void UBEBehaviorTreeGraph::UpdateVersion_UnifiedSubNodes()
{
	for (int32 NodeIdx = 0; NodeIdx < Nodes.Num(); NodeIdx++)
	{
		UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(Nodes[NodeIdx]);
...
void UBEBehaviorTreeGraph::UpdateVersion_InnerGraphWhitespace()
{
	for (int32 NodeIdx = 0; NodeIdx < Nodes.Num(); NodeIdx++)
	{
		UBEBehaviorTreeGraphNode* MyNode = Cast<UBEBehaviorTreeGraphNode>(Nodes[NodeIdx]);
		if (MyNode)
		{
			...
			{
				UBEBehaviorTreeGraphNode_CompositeDecorator* InnerGraphNode = Cast<UBEBehaviorTreeGraphNode_CompositeDecorator>(MyNode->SubNodes[SubIdx]);
